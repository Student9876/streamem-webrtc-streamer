...
// Helper function to get server URL with dynamic port
const getServerUrl = () => {
    // Use the SERVER_PORT variable set by the main process
    const port = 3001;
...
const connectSocket = (retries = 3): Promise<Socket> => {
    return new Promise((resolve, reject) => {
        ...
        const tryConnect = () => {
            ...
            console.log(`Attempting to connect to socket.io server at ${serverUrl} (attempt ${attempts}/${retries})`);
...
export default function Host() {
    ...
    // Debug useEffect to check environment and server status
    useEffect(() => {
    ...
    const startStream = async () => {
        try {
            ...
            try {
                ...
                setDebugInfo(prev => prev + `\n❌ Failed to connect to signaling server at ${getServerUrl()}. Is the server running?`);
